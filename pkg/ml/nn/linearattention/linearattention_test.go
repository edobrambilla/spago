...
func TestModel_SelfAttention(t *testing.T) {
	...
	if !floats.EqualApprox(output[0].Value().Data(), []float64{0.68021652, -0.4026615, -0.437588203}, 1.0e-05) {
		t.Error("The output doesn't match the expected values")
	}
	if !floats.EqualApprox(output[1].Value().Data(), []float64{0.6787047, -0.3854853, -0.43176143}, 1.0e-05) {
		t.Error("The output doesn't match the expected values")
	}
	if !floats.EqualApprox(output[2].Value().Data(), []float64{0.6720585, -0.3838545, -0.44173931}, 1.0e-05) {
		t.Error("The output doesn't match the expected values")
	}
